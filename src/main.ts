...
const targetCurrencySelector = document.querySelector(
  '#target-currency'
) as HTMLSelectElement;

// A global variable that references the HTML paragraph element with the id conversion-result
const resultSelector = document.querySelector(
  '#conversion-result'
...
const getConversionRates = async () => {
  const base = baseCurrencySelector.value;
  const target = targetCurrencySelector.value;
...
baseCurrencySelector.addEventListener('change', getConversionRates);
...
targetCurrencySelector.addEventListener('change', getConversionRates);
...
const currencyConversionRate = (
  ...
  if (conversionInfo) {
    ...
    resultSelector.textContent = `1 ${baseCurrencySelector.value} = ${conversionRate} ${currencyName}`;
  } else {
    resultSelector.textContent = 'Conversion rate not found';
  }
};
